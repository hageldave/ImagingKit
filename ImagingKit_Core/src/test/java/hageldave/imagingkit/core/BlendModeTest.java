...
public class BlendModeTest {
	...
	public void testConsumers() {
		...
		imgB.forEach(BlendMode.NORMAL.getBlendingWith(imgT));
		...
		imgB.fill(opaqueBlack);
		// test offset
		imgB.forEach(BlendMode.NORMAL.getBlendingWith(imgT, 5, 5));
		imgB.forEach(px->{
		...
		imgB.forEach(BlendMode.NORMAL.getBlendingWith(imgT, 0.5f));
		for(int c: imgB.getData()){
			assertTrue(c != opaqueBlack && c != opaqueWhite);
		}
		imgB.fill(opaqueBlack);
		// test visibility and positive and negative offset
		imgB.forEach(BlendMode.NORMAL.getBlendingWith(imgT, -5, -5, 0.5f));
		imgB.forEach(BlendMode.NORMAL.getBlendingWith(imgT,  5,  5, 0.5f));
		imgB.forEach(px->{
			if(px.getX() < 5 && px.getY() < 5){
