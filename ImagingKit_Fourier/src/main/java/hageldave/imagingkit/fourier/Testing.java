...
//		});
	...
	static void shift2D__(double[] a, int w, int h, int x, int y){
		assertPositive(w, ()->"specified width is not positive. w="+w);
		assertPositive(h, ()->"specified height is not positive. h="+h);
		while(x < 0) x = w+x;
		while(y < 0) y = h+y;
		...
		for (int cycleStart = 0, nMoved = 0; nMoved != size; cycleStart = (cycleStart+w+1)%size) {
			double displaced = a[cycleStart];
			int i = cycleStart;
			do {
				i = shiftedIndex(i, w, h, x, y);
				double t=a[i];
				a[i]=displaced;
				displaced=t;
				nMoved ++;
